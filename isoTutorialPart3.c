/*
 *   Isometric Game Tutorial Part 3
 *   Author: Johan Forsblom
 *   Written between: 28 Jan 2018 and 2st of April 2018
 *
 *   Cleaned up and memory tested with valgrind in linux before released
 *
 *   This tutorial covers the following:
 *
 *      * Refactoring the zoom in / out code
 *      * Refactoring texture.c
 *      * Texture pool
 *      * Change texture loading to work with texture pool
 *      * Initialize memory of isometric map - missing since earlier tutorials
 *      * Add a function to convert an isometric point to Cartesian coordinates: isoEngineConvertIsoPoint2DToCartesian(...)
 *      * Move Game mode into the isometric engine
 *      * Add Delete log file function to the logger
 *      * Delta timer
 *          - Adjust isoEngineScrollMapWithMouse() to work with delta timer
 *          - Adjust default map scroll speed
 *      * Update isoGenerateMap(isoMapT *isoMap) to draw some blocks on the 2nd layer so we can test entity depth sorting
 *      * Depth sorting on-screen entities and rendering them in-order on the isometric map
 *
 *      * Entity Component System (ECS)
 *          Entity                      - ID for an entity and the components attached to it
 *          Components                  - Stores entity data
 *           - Position Component       - Data for an entity's position
 *           - Velocity Component       - Data for an entity's velocity and friction
 *           - Render2D Component       - Data for rendering an entity
 *           - Keyboard input Component - Stores keyboard actions for an entity
 *           - Mouse input Component    - Stores mouse actions for an entity
 *           - Name tag Component       - Stores the name of the entity
 *          Systems                     - Manipulates the entity data
 *           - Move system              - System for moving entities in the game world
 *           - Input system             - System for updating mouse and keyboard events for entities (Gamepad will come later).
 *           - Control entity system    - System for making entities react to mouse and keyboard input.
 *           - Control iso world system - System for making the isometric world react to mouse and keyboard input
 *           - Render iso world system  - System for rendering the isometric world and the entities in it
 *          Scene                       - A game scene. Contains entities, components and systems
 *          ECS Scene Manager           - Handles scenes and functionality for switching between them
 *      * Refactoring the code to work with the Entity Component System
 *      * Refactoring the isoEngineCenterMap(...) to work with the Entity Component System
 *      * Frames/second counter to measure performance
 *
 *  Usage:
...
/// TREES ON THE MAP!
    ...
    for(i=0;i<200;++i)
    {
        ...
        entity = ecsSceneAddEntityToScene(testScene,
             COMPONENT_SET1_POSITION | COMPONENT_SET1_NAMETAG | COMPONENT_SET1_RENDER2D | COMPONENT_SET1_VELOCITY);
